...
# not finished
def getROIcoord(img,left_down_x,left_down_y,right_down_x,right_down_y):

    # for now look only at a square
    # later take into distance to tower and actual tower height

    tower_height = int(0.5*img.shape[0])    # temporary
    tower_width =  right_down_x - left_down_x         # in pixel space
    left_up_y = img.shape[0] - left_down_y - tower_height
    right_up_y = img.shape[0] - right_down_y - tower_height
    return left_up_y,right_up_y


# not finished
def isInROI(img,blob_center_x,blob_center_y,left_down_x,left_down_y,right_down_x,right_down_y):
    # should take width as well in the future?
    # assume for now that x coords of up corners are the same as bottom corners

    cond1 = False
    cond2 = False
    cond3 = False
    left_up_y, right_up_y = getROIcoord(img,left_down_x,left_down_y,right_down_x,right_down_y)


    if (left_down_x < blob_center_x) and (blob_center_x < right_down_x):
        cond1 = True
    if (blob_center_y < left_down_y) and (left_up_y < blob_center_y):
        cond2 = True
    if (blob_center_y < right_down_y) and (right_up_y < blob_center_y):
        cond3 = True
...
def getContours(color_masks,colors):
    ...
    for m in range(len(color_masks)):
        ...
        if len(cntsR) != 0:
            ...
            #print(colors[m]+' disk was not found!') # uncomment later
            pass
...
def hsvConvert(baxter_hsv_offset):
    # baxter_hsv_offset should be between [0,255]
    actual_hsv_offset = baxter_hsv_offset / 255.0   # [0,1]
    actual_hsv_offset = int(actual_hsv_offset *10)  # [0,10]
    return actual_hsv_offset
...
def gammaConvert(baxter_gamma):
    """Converts a wheel value between [0,255] to a gamma value between [-2,2]"""

    # baxter_gamma shoud be  value between [0,255]
    actual_gamma = baxter_gamma / 255.0     # [0,1] (float division for proper mapping)
    actual_gamma = actual_gamma - 0.5       # [-0.5,0.5]
    actual_gamma = actual_gamma * 4         # [-2,2]
    return actual_gamma
...
def tileConvert(baxter_tile):
    """Converts a wheel value between [0,255] to a tile_size value between [1,40]"""

    # baxter_tile shoud be  value between [0,255]
    actual_tile = baxter_tile / 255.0       # [0,1] (float division for proper mapping)
    actual_tile = int(actual_tile * 40)     # [0,40]
...
def preprocessImage(img,gamma,tile_size, wb):
    ...
    img = clahe(img,tile_size) # tile size = 5x5
...
def getBGRavgColorsFromBaxter(img, color_names, save_masks):
    ...
    disk_height = int(img_h/(nr_disks+30))      # disk height (trial and error)
    disk_width = int(img_w/30)                  # disk width
    min_y = img_margin + int(disk_height/2)     # minimum y coord of a disk
    max_y = img_h - min_y                       # maximum y coord of a disk
    inter_space = 15                            # distance between ellipses (trial and error)
...
def saveHSV(colors_dictionary_hsv):
    # path = os.path.join('src','hanoi','src','calibration_data','hsv.txt')
    path = os.path.join('calibration_data', 'hsv.txt')
    print(path)
    file = open(path,'w')
    for color_name, hsv_value in colors_dictionary_hsv.iteritems():
        low = hsv_value[0]
        up = hsv_value[1]
        file.write(color_name)
        for i in range(3):
            file.write(' ')
            file.write(str(low[i]))
...
def saveProcessParams(white_balance,gamma,tile_size):
    # order of writing: white_balance,gamma,tile_size
    # path = os.path.join('src','hanoi','src','calibration_data','preprocess.txt')
    path = os.path.join('calibration_data', 'preprocess.txt')
    file = open(path, 'w')
    file.write(str(white_balance)+',')
    file.write(str(gamma)+',')
    file.write(str(tile_size))
    file.close()


def loadProcessParams():
    # order of reading: white_balance,gamma,tile_size
    # path = os.path.join('src','hanoi','src','calibration_data','preprocess.txt')
    path = os.path.join('calibration_data', 'preprocess.txt')
    file = open(path, 'r')
    for line in file:
        info = line.split(',')
        white_balance = bool(info[0])
        gamma = float(info[1])
        tile_size = int(info[2])
    file.close()
    return white_balance,gamma,tile_size
...
def loadHSV():
    ...
    for line in file:
        ...
        info = line.split(' ')
        color_name = info[0]
        color_min = np.array([info[1], info[2], info[3]], np.uint8)
        color_max = np.array([info[4], info[5], info[6]], np.uint8)
        hsv_baxter[color_name]= [color_min,color_max]
    file.close()
    return hsv_baxter
...
def drawBoxes(img,contours,found_colors):
    ...
    # cv2.imshow('Masks', stacked_masks)
    # cv2.waitKey(1)
    return stacked_masks,img


def showParamOnImg(img,hsv_offset,white_balance,gamma,tile_size):
    h_string = 'Hue range: '+str(hsv_offset)
    wb_string = 'White-balance: '+str(white_balance)
    g_string = 'Gamma: '+str(gamma)
    t_string = 'Tile size: '+str(tile_size)

    # # image, text, bottom-left corner of the text, font, font scale, color, thickness
    cv2.putText(img, h_string, (0, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    cv2.putText(img, wb_string, (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    cv2.putText(img, g_string, (0, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    cv2.putText(img, t_string, (0, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    # cv2.imshow('B',img)
    return img


def vote(board_states_queue):
    board_states_queue = Queue(maxsize=5)
    while not board_states_queue.empty():
        state = board_states_queue.get()
